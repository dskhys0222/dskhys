...
sequenceDiagram
    ...
    alt メールアドレスが既に存在
        ...
        Client->>Client: トークンを保存<br/>(SessionStorage/SecureStorage)
...
- **アクセストークン**: 
  - Web: SessionStorage（XSS対策のため、短期間のみ保存）
  - アプリ: セキュアストレージ（Keychain/Keystore等）
  - ※ Authorization ヘッダーでトークンを送信するため、HTTPOnly Cookie は使用しない
- **リフレッシュトークン**: 
  - Web: SessionStorage（XSS対策のため、短期間のみ保存）
  - アプリ: セキュアストレージ（Keychain/Keystore等）
  - ※ より安全な方法として、サーバー側でセッション管理を行い、クライアントにはセッションIDのみを返す方式も検討可能
